...
ss_conc16_18_clean1 <- ss_conc16_18_orig %>% 
  ...
  filter(Result != "Not analyzed") %>% 
  # convert one result with a value of "." to "Non-detect"
  mutate(Result = if_else(Result == ".", "Non-detect", Result))
...
The contaminant suspended sediment concentration data from 2016-2018 and 2019 can now be combined into one dataframe. We will also need to standardize the "Site" variable and convert the "Date" variable to a date object so that the MDL values from the data downloaded from NWIS can be joined correctly.
...
```{r look at duplicate mdl val susp sed}
# Create a vector of the parameters with 2 MDL values during the period of record
...
ss_conc_all2_estMDL2 <- ss_conc_all2 %>% 
  ...
  mutate(
    rpt_lev_va = if_else(year(Date) <= 2016, 3.2, 8.3)
  )
...
* Creating a new variable for the measurement units
* Making the Result variable equal to its MDL value for the samples below the MDL
* Rounding the Result and MDL variables to 1 significant figure for values less than 1, 2 significant figures for values less than 10 and 3 significant figures for values greater than 10
* Changing some variable names to standardized names for the NDFA synthesis
...
ss_conc_all_f <- ss_conc_all4 %>% 
  ...
  # Round the Result and MDL variables to the appropriate number of significant figures
  round_val(Result) %>% 
  round_val(rpt_lev_va) %>% 
  # add "Estimated" category to the LabDetect variable for reported values less than the MDL
  mutate(LabDetect = if_else(Result < rpt_lev_va, "Estimated", LabDetect)) %>% 
  # standardize variable names
  select(
    StationCode = Site,
    Date,
...
Export the contaminant suspended sediment concentration data as a .csv file to the Processed_Data/Contaminants folder on SharePoint.
...
ss_conc_all_f %>% 
  write_excel_csv(
    path = paste0(fp_abs_proc, "/WQ_OUTPUT_Contam_SuspSed_formatted.csv"),
    na = ""
  )
```
