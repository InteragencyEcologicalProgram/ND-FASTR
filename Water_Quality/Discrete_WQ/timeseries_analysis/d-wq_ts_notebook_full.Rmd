...
create_facet <- function(df){
  ...
  if (any_dat) {
    ...
    df_filt <- df_filt[,y_min := min(Result, na.rm = TRUE) - 0.01, by = Region]
    df_filt <- df_filt[,y_max := max(Result, na.rm = TRUE) + .01, by = Region]
...
fp_rel_hypo <- paste0(fp_fastr,'WQ_Subteam/Processed_Data/Discrete/Discrete_Hypothesis_Table.csv')
