...
create_facet <- function(df){
  ...
  if (any_dat) {
    ...
    p <- p +
      ...
      geom_point( # points
        df_filt,
        mapping = aes(x = Date, y = Result),
        color = '#fcce25',#StationCode),
        size = 2.8
      ) +
      geom_line( # line
        df_filt,
    ...
    if (rl_dat) {
      p <- p +
        geom_segment( # vertical segment
          data = df_seg,
          mapping = aes(x = x_vert, xend = xend_vert, y = y_vert, yend = yend_vert),
          color = '#fcce25',#StationCode),
          size = 0.8
        ) +
        geom_segment( # horizontal segment
          data = df_seg,
          mapping = aes(x = x_horz, xend = xend_horz, y = y_horz, yend = yend_horz),
          color = '#fcce25',#StationCode),
          size = .6,
          lineend = 'square'
        )
...
```{r, message=FALSE, warning=FALSE, results='asis', echo=FALSE, fig.width=8, fig.height=3, fig.align='center'}
