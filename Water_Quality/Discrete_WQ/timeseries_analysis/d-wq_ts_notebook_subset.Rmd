...
create_facet <- function(df){
  ...
  if (any_dat) {
    ...
    if (rl_dat) {
      ...
      p <- p +
        geom_segment( # vertical segment
          data = df_seg,
          mapping = aes(x = x_vert, xend = xend_vert, y = y_vert, yend = yend_vert, group = StationCode, color = BroadRegion),
          size = 1.1
        ) +
        geom_segment( # horizontal segment
          data = df_seg,
          mapping = aes(x = x_horz, xend = xend_horz, y = y_horz, yend = yend_horz, group = StationCode, color = BroadRegion),
          size = .8,
          lineend = 'square'
        )
    }
    ...
    p <- p +
      geom_point( # points
        df_filt,
        mapping = aes(x = Date, y = Result, group = StationCode, color = BroadRegion),
        size = 3.0
      ) +
      geom_line( # line
        df_filt,
      ...
      geom_point(data = df_filt, aes(y = y_max, x = Date, color = NA)) +
