...
# Create plots for each year-analyte combination
df_dwq_plots <- df_dwq_clean %>% 
  nest(data = -c(Year, Analyte)) %>% 
  mutate(plot = map(data, .f = create_dwq_plot)) %>% 
  nest(data2 = -Year)
```
...
```{r print plots, echo = FALSE, results = "asis", fig.height = 9, fig.width = 8}
for (i in 1:nrow(df_dwq_plots)) {
  # Create heading for each parameter
  cat("## ", as.character(df_dwq_plots$Year[i]), " {.tabset .tabset-pills}\n\n")
  for (j in 1:nrow(df_dwq_plots$data2[[i]])) {
    # Create subheadings for each yr
    cat("### ", as.character(df_dwq_plots$data2[[i]]$Analyte[j]), "\n\n") 
    # Display plot
    print(df_dwq_plots$data2[[i]]$plot[[j]])
    cat("\n\n")  
  }
}
```

# Counts of Sampling Events

```{r create datatable}
df_dwq_clean %>% 
  count(Analyte, StationCode, Year, ActionPhase) %>% 
