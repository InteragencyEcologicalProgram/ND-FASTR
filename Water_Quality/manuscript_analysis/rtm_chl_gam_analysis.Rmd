...
df_rtm_chla_c1 <- df_rtm_chla %>% 
  ...
  mutate(
    StationCode = fct_drop(StationCode),
    Year_fct = fct_drop(Year_fct)
  )
...
appraise(m_chla_gam)
k.check(m_chla_gam)
plot(m_chla_gam, pages = 1, residuals = TRUE)
...
plot(m_chla_gam_k5, pages = 1, residuals = TRUE)
...
plot(m_chla_gam_lag1, pages = 1, residuals = TRUE)
...
plot(m_chla_gamm_k5_ar3_gam, residuals = TRUE)
...
plot(m_chla_gamm_k5_ar3b_gam, residuals = TRUE)
...
plot(m_chla_gam_st, pages = 1, residuals = TRUE)
...
plot(m_chla_gam_st_k5, pages = 1, residuals = TRUE)
...
df_rtm_chla_c2 <- df_rtm_chla_c1 %>% filter(!StationCode %in% c("RCS", "RYI"))
...
plot(m_chla_gamm_st_k5_ar2_gam, residuals = TRUE)
...
emmeans(m_chla_gamm_st_k5_ar2, specs = pairwise ~ FlowActionPeriod | Year_fct, data = df_rtm_chla_c2)

# Contrasts in FlowActionPeriod for each Station
emmeans(m_chla_gamm_st_k5_ar2, specs = pairwise ~ FlowActionPeriod | StationCode, data = df_rtm_chla_c2)

# Contrasts in Station for each Year
multcomp::cld(
  emmeans(m_chla_gamm_st_k5_ar2, specs = pairwise ~ StationCode | Year_fct, data = df_rtm_chla_c2)$emmeans,
  sort = FALSE,
  Letters = letters
)
...
multcomp::cld(
  emmeans(m_chla_gamm_st_k5_ar2, specs = pairwise ~ Year_fct | StationCode, data = df_rtm_chla_c2)$emmeans,
  sort = FALSE,
  Letters = letters
)
```
