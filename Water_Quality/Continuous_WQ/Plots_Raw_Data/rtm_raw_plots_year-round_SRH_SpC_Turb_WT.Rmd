---
title: "Raw Data Plots - SRH Year-round (SpCnd, Turbidity, WaterTemp)"
...
Create plots of the raw continuous water quality data (SpCnd, Turbidity, and WaterTemp) collected year-round at the SRH station. All data in these plots has been cleaned and formatted, but not all of it has been inspected for suspect values. Data has already been QA'ed and validated for the time period during the flow actions and 45 days before and after. Data outside of these time periods is still considered raw and not validated. The purpose of these plots is to allow for us to visually inspect the raw data for suspect or unreliable values.
...
```{r define file paths}
# Define relative file path for processed continuous WQ data file for SRH - year-round data
fp_rel_rtm_proc_all <- 
  "WQ_Subteam/Processed_Data/Continuous/All_Dates/RTM_OUTPUT_SRH_formatted_all.csv"
...
```{r import data}
# Import processed continuous WQ data for SRH - year-round data
rtm_wq_proc_orig <- import_rtm_data(fp_abs_rtm_proc_all, 6)
...
rtm_wq_proc_clean <- rtm_wq_proc_orig %>% 
  ...
  # only keep a subset of the parameters (SpCnd, Turbidity, WaterTemp)
  select(DateTime, SpCnd, Turbidity, WaterTemp) %>%
