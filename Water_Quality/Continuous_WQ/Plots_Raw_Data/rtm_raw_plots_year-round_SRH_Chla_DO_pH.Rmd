---
title: "Raw Data Plots - SRH Year-round (Chla, DO, pH)"
...
Create plots of the raw continuous water quality data (Chla, DO, and pH) collected year-round at the SRH station. All data in these plots has been cleaned and formatted, but not all of it has been inspected for suspect values. Data has already been QA'ed and validated for the time period during the flow actions and 45 days before and after. Data outside of these time periods is still considered raw and not validated. The purpose of these plots is to allow for us to visually inspect the raw data for suspect or unreliable values.
...
rtm_wq_proc_clean <- rtm_wq_proc_orig %>% 
  ...
  # only keep a subset of the parameters (Chla, DO, and pH)
  select(DateTime, Chla, DO, pH) %>% 
