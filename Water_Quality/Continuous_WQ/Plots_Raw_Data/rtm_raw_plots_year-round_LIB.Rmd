---
title: "Raw Data Plots - LIB Year-round"
...
Create plots of the raw continuous water quality data collected year-round at the LIB station. All data in these plots has been cleaned and formatted, but not all of it has been inspected for suspect values. Data has already been QA'ed and validated for the time period during the flow actions and 45 days before and after. Data outside of these time periods is still considered raw and not validated. The purpose of these plots is to allow for us to visually inspect the raw data for suspect or unreliable values.
...
```{r define file paths}
# Define relative file path for processed continuous WQ data file for LIB - year-round data
fp_rel_rtm_proc_all <- 
  "WQ_Subteam/Processed_Data/Continuous/All_Dates/RTM_OUTPUT_LIB_formatted_all.csv"
...
```{r import data}
# Import processed continuous WQ data for LIB - year-round data
rtm_wq_proc_orig <- import_rtm_data(fp_abs_rtm_proc_all, 11)
...
We need to remove some Chla data in 2018 that has already been flagged and excluded in the QA'ed data set but was at the beginning of the filtered date period.
```{r remove flagged data}
rtm_wq_all_clean <- rtm_wq_all %>% 
  filter(
    !(Parameter == "Chla" & DateTime >= "2018-07-13 00:00:00" & DateTime <= "2018-08-14 06:15:00")
  )
```

```{r prepare all data}
# Finish preparing all joined data for plotting
rtm_wq_all_clean_v1 <- rtm_wq_all_clean %>% 
  # add a year variable
...
rtm_wq_plots <- rtm_wq_all_clean_v1 %>% mutate(plt = map2(df_data, Parameter, create_ts_plotly)) 
```
