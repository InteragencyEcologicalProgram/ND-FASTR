...
# Define relative file path for processed continuous WQ data file for TOE
fp_rel_rtm_data <- paste0(fp_fastr, "Processed_Data/Continuous/RTM_OUTPUT_TOE_formatted.csv")
...
```{r import data}
# Import data
data_orig <- import_rtm_data(fp_abs_rtm_data, 10)
```
...
data_all_yrs <- data_clean %>% 
  ...
  # remove NA values for FlowTF so that it plots correctly
  mutate(
    data = if_else(
      parameter == "FlowTF",
      map(data, ~filter(.x, !is.na(value))),
      data
    )
  )
