---
title: "Raw Data Plots - STTD Year-round"
...
Create plots of the raw continuous water quality data collected year-round at the STTD station. All data in these plots has been cleaned and formatted, but not all of it has been inspected for suspect values. Data has already been QA'ed and validated for the time period during the flow actions and 45 days before and after. Data outside of these time periods is still considered raw and not validated. The purpose of these plots is to allow for us to visually inspect the raw data for suspect or unreliable values.
...
```{r define file paths}
# Define relative file path for processed continuous WQ data file for STTD - year-round data
fp_rel_rtm_proc_all <- 
  "WQ_Subteam/Processed_Data/Continuous/All_Dates/RTM_OUTPUT_STTD_formatted_all.csv"
...
```{r import data}
# Import processed continuous WQ data for STTD - year-round data
...
rtm_wq_all_clean <- rtm_wq_all %>% 
  ...
  # remove data before 2016 since it wasn't collected year-round before then
  filter(!Year < 2016) %>% 
  # remove data with NA values
  filter(!is.na(Value)) %>% 
