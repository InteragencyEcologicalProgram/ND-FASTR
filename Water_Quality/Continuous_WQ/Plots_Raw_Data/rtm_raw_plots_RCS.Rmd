...
```{r define file paths}
# Define relative file path for processed continuous WQ data file for RCS
fp_rel_rtm_data <- 
  "WQ_Subteam/Processed_Data/Continuous/Filtered_Dates/RTM_OUTPUT_RCS_formatted_filt.csv"
...
```{r print chla do flow ph and spcnd plots, include = FALSE}
# Create a list of plot templates to use for Chla, DO, Flow, pH, and SpCnd plots
produce_plots_chla_do_flow_ph_spcnd <- list(
  knit_expand(file = "rtm_raw_plots_template_2014.Rmd", parameter = "Chla"),
  knit_expand(file = "rtm_raw_plots_template_2014.Rmd", parameter = "DO"),
  knit_expand(file = "rtm_raw_plots_template_2011.Rmd", parameter = "Flow"),
  knit_expand(file = "rtm_raw_plots_template_2014.Rmd", parameter = "pH"),
  knit_expand(file = "rtm_raw_plots_template_2014.Rmd", parameter = "SpCnd")
)
```

`r knit(text = unlist(produce_plots_chla_do_flow_ph_spcnd))`


## Turbidity {.tabset .tabset-pills}

```{r filter indiv turb plots, include = FALSE}
...
```{r plot turb all yrs, echo = FALSE}
df_plots_all_yrs %>% 
...
```{r print water temp plots, include = FALSE}
# Create a list of the plot template to use for WaterTemp plots
produce_plots_wt <- list(
  knit_expand(file = "rtm_raw_plots_template_2014.Rmd", parameter = "WaterTemp")
)
```

`r knit(text = unlist(produce_plots_wt))`
