...
fp_rel_rtm_rd22 <- paste0(fp_fastr, "Processed_Data/Continuous/RTM_OUTPUT_RD22_formatted.csv")

# Define absolute file path
fp_abs_rtm_rd22 <- get_abs_path(fp_rel_rtm_rd22)
```
...
```{r import data}
rd22_orig <- import_rtm_data(fp_abs_rtm_rd22, 7)
```
...
rd22_yrs_c <- 
  bind_rows(rd22_all_yrs, rd22_indiv_yrs) %>% 
...
for (i in 1:nrow(rd22_plots)) {
  # Create heading for each parameter
  cat("## ", as.character(rd22_plots$parameter[i]), " {.tabset .tabset-pills}\n\n")
  for (j in 1:nrow(rd22_plots$data[[i]])) {
    # Create subheadings for each yr
    cat("### ", as.character(rd22_plots$data[[i]]$yr[j]), "\n\n") 
    # Display plot
    print(rd22_plots$data[[i]]$plot[[j]])
    cat("\n\n")  
  }
}
```
