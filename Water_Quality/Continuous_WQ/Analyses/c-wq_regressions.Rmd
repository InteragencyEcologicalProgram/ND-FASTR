...
# create lags
df_wq_day <- slide(df_wq_day, 'Result', TimeVar = 'WeekYear', GroupVar = c('StationCode','Analyte','Year'), NewVar = 'lag1', slideBy = -1)
df_wq_day <- slide(df_wq_day, 'lag1', TimeVar = 'WeekYear', GroupVar = c('StationCode','Analyte','Year'), NewVar = 'lag2', slideBy = -1)
df_wq_day <- slide(df_wq_day, 'lag2', TimeVar = 'WeekYear', GroupVar = c('StationCode','Analyte','Year'), NewVar = 'lag3', slideBy = -1)
...
for (i in seq(analytes)){
  ...
  lmer_fit <- lmer(Result ~ Month + Year + FlowActionPeriod + BroadRegion + (1|StationCode), data = df_temp)
...
# write_csv(all_aov_data, ndfa_abs_sp_path('WQ_Subteam/Analysis_Results/CWQ/no_SDI/cwq_aov_noSDI_0.csv'))
```
