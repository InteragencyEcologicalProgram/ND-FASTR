...
for (ana in analytes) {
  ...
  # Run model without any seasonal terms
  lmer_fit <- 
    lmer(
  ...
  acf(residuals(lmer_fit), na.action = na.pass, main = ana)
  ...
  # Run model with month as seasonal term
  lmer_fit_month <- 
    lmer(
      Result ~ Year + FlowActionPeriod + BroadRegion + Month + lag1 + lag2 + (1|StationCode), 
     data = df_temp
   )
  # Create and save ANOVA Diagnostics plots
  pdf(paste0(fp_anova_diag, "/", ana, "_daily_avg_sea-month.pdf"))
  acf(residuals(lmer_fit_month), na.action = na.pass, main = ana)
  plot(residuals(lmer_fit_month))
  qqnorm(residuals(lmer_fit_month))
  hist(residuals(lmer_fit_month), breaks = 15)
  dev.off()
  
  # Run model with week as seasonal term
  lmer_fit_week <- 
    lmer(
      Result ~ Year + FlowActionPeriod + BroadRegion + Week + lag1 + lag2 + (1|StationCode), 
     data = df_temp
   )
  # Create and save ANOVA Diagnostics plots
  pdf(paste0(fp_anova_diag, "/", ana, "_daily_avg_sea-week.pdf"))
  acf(residuals(lmer_fit_week), na.action = na.pass, main = ana)
  plot(residuals(lmer_fit_week))
  qqnorm(residuals(lmer_fit_week))
  hist(residuals(lmer_fit_week), breaks = 15)
  dev.off()
}
```
