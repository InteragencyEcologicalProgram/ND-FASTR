---
title: "Test outlier detection methods on NDFA RTM data - Chla"
...
```{r run methods chla, message = FALSE}
  ...
  mutate(
    odwg_test_calc = map(
      ...
      ~mutate(
        ...
        qual_spike_fixed = rtqc_spike(value, c(15, 30)),
...
df_outlier_test_plots <- df_outlier_test %>% 
  mutate(
    plot_spike_fixed = map(
      ...
      plot_title = "Spike test - 15 and 30"
    ),
    plot_spike_dyn = map(
      odwg_test_f, 
      .f = outlier_test_plot, 
...
# Visualize Chla results {.tabset .tabset-pills}
