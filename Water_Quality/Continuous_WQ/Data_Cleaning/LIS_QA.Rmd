...
df_LIS <- df_LIS %>%
  mutate(
    ...
    # case_when(DateTime > '2014-08-04 1:00:00' & DateTime < '2014-08-05 10:15:00' ~ NA),  # figure it out later
...
```{r}
# check via plots
df_plot <- df_LIS %>% filter(year(DateTime) == 2014)

plt <- ggplot(data = df_plot, aes(x = DateTime, y = Turbidity)) + geom_line()
