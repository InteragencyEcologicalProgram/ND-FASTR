...
Most of the continuous water quality data collected by USGS was [downloaded from NWIS](https://github.com/InteragencyEcologicalProgram/ND-FASTR/blob/master/Water_Quality/Continuous_WQ/USGS_RTM_Data_Download.R). However, Jared Frantzich (DWR NCRO) shared some additional USGS data from the LIB, RYI, and SDI stations that was not publicly available. For this additional data, we are only interested in the flow data for SDI, but we are interested in all available parameters for LIB and RYI. We will import the publicly available and non-publicly available data separately.

```{r import raw usgs data}
# Create a vector of all file paths for the raw USGS continuous data
usgs_fp <- sort(dir(fp_abs_wq_raw, pattern = "USGS", full.names = T))

# Remove the three .xlsx files with non-publicly available USGS data from LIB, RYI, and SDI since we will import those separately
usgs_fp_nwis <- str_subset(usgs_fp, "JF.xlsx$", negate = TRUE)

# Create a tibble of all raw USGS continuous data file paths - downloaded from NWIS
usgs_files_nwis <- tibble(
  filepath = usgs_fp_nwis,
  n_params = c(18, 10, 4, 6, 12, 10, 11, 7, 10, 3)
)

# Import USGS data that was downloaded from NWIS into a nested dataframe
usgs_nwis_orig <- usgs_files_nwis %>% 
  mutate(
...
# Import the three .xlsx files with non-publicly available USGS data from LIB, RYI, and SDI
lib_xlsx_orig <- read_excel(
  file.path(fp_abs_wq_raw, "RTM_RAW_USGS_LIB_2013_18_JF.xlsx"),
  col_types = c(
    "skip",
    "date",
    rep("skip", 9),
...
ryi_xlsx_orig <- read_excel(
  file.path(fp_abs_wq_raw, "RTM_RAW_USGS_RYI_2016_JF.xlsx"),
  col_types = c(
    "skip",
    "date",
    rep("skip", 8),
...
sdi_xlsx_orig <- read_excel(
  file.path(fp_abs_wq_raw, "RTM_RAW_USGS_SDI_2011-2018_JF.xlsx"),
  col_types = c(
    "skip",
    "date",
    rep("skip", 5),
...
### Prepare three xlsx files (LIB, RYI, and SDI)

We need to prepare the three .xlsx files with non-publicly available USGS data from LIB, RYI, and SDI so that they can be combined with the remaining USGS data later in this data processing script.

```{r prepare three xlsx files usgs}
# Rename variables in the data frames for LIB and RYI
usgs_xlsx_clean_v1 <- 
  tibble(
    sta_code = c("LIB", "RYI"),
    df = list(lib_xlsx_orig, ryi_xlsx_orig)
  ) %>% 
  mutate(
    df = map(
      df, 
      ~rename(
        ...
        WaterTemp = "Temp (deg C)",
        SpCnd = "SPC (uS/cm)",
        DO = "DO (mg/L)",
        fDOM = "CDOM (QSE)",
        Chla = "Chl-a (ug/L)",
        Turbidity = "Turb (FNU)",
        NitrateNitrite = "Nitrate (mg/L)"
      )
    )
  )

# Rename variables in the data frame for SDI
sdi_xlsx_clean <- sdi_xlsx_orig %>% 
  rename(
    DateTime = Timestamp,
    Flow = "Q (cfs)",
    FlowTF = "Q Tidal Filt (cfs)"
...
# Add SDI data frame to the nested data frame with LIB and RYI
usgs_xlsx_clean_v1 %<>% 
  add_row(
    sta_code = "SDI",
    df = list(sdi_xlsx_clean)
  )
```

### Parse date-time variables

The date-time variables of the USGS water quality data downloaded from NWIS need to be converted from character to datetime objects. The date-time variables in the three .xlsx files with non-publicly available USGS data are already datetime objects upon import of the data. All datetimes need to be forced as PST.

```{r clean date time var usgs}
# Parse DateTime variables of the USGS water quality data as datetime objects - downloaded from NWIS
usgs_nwis_clean_v1 <- usgs_nwis_orig %>% 
  mutate(
    df = map(
      df, 
...
# Define timezone of DateTime as PST for the three .xlsx files with non-publicly available USGS data
usgs_xlsx_clean_v2 <- usgs_xlsx_clean_v1 %>% 
  mutate(df = map(df, ~mutate(.x, DateTime = force_tz(DateTime, tzone = "Etc/GMT+8"))))
```

### Remove rows with all missing values

Before we proceed with the rest of the data cleaning, we need to remove the rows where all of the data values are missing.

```{r remove missing data usgs, message = FALSE}
usgs_nwis_clean_v2 <- usgs_nwis_clean_v1 %>% 
  mutate(df = map(df, ~filter(.x, !if_all(where(is.numeric), is.na))))

usgs_xlsx_clean_v3 <- usgs_xlsx_clean_v2 %>% 
  mutate(df = map(df, ~filter(.x, !if_all(where(is.numeric), is.na))))
```

### Remove overlapping or duplicated data

Some continuous USGS stations had more than one project collecting data at the same location and time. Because of this, we need to look for any overlapping or duplicated data and clean it. This only applies to the data downloaded from NWIS.

```{r look for overlapping data usgs}
usgs_nwis_qc_check <- usgs_nwis_clean_v2 %>% 
  mutate(
    df_count = map(
      df, 
...
unique(usgs_nwis_qc_check$df_count[[1]]$parameter)

# Summarize time period with overlapping data
summary(usgs_nwis_qc_check$df_count[[1]])
```

There are multiple parameters (DO, fDOM, pH, SpCnd, Turbidity, WaterTemp, and Chla) that have overlapping data in duplicate at LIB from 9/27/2018 to 10/10/2018. After looking at LIB (`r unique(usgs_nwis_qc_check$df[[1]]$site_no)`) on the [USGS NWIS website](https://waterdata.usgs.gov/nwis/uv?site_no=11455315), the cause of the overlapping data is from a short-term Chlorophyll inter-calibration project. We will remove the data collected for this short-term project to avoid overlapping data in the dataset.

```{r clean lib data}
# LIB - remove data from the Chlorophyll inter-calibration project
lib <- usgs_nwis_clean_v2 %>% 
  filter(sta_code == "LIB") %>% 
  pull(df) %>% 
  chuck(1)
...
# Find parameters with overlapping data
unique(usgs_nwis_qc_check$df_count[[7]]$parameter)

# Summarize time period with overlapping data
summary(usgs_nwis_qc_check$df_count[[7]])
```

There are two parameters (SpCnd and WaterTemp) that have overlapping data in duplicate at SDI from 1/25/2013 to 10/3/2013. After looking at SDI (`r unique(usgs_nwis_qc_check$df[[7]]$site_no)`) on the [USGS NWIS website](https://waterdata.usgs.gov/nwis/uv?site_no=11455478), the cause of the overlapping data is from two projects (Hydro Exo and BGC Project) collecting SpCnd and WaterTemp data during this time period. We will only keep the data collected for the BGC Project during this time period to avoid overlapping data in the dataset.

```{r clean sdi data, message = FALSE}
# SDI - remove data from the Hydro Exo project after 1/24/2013
sdi <- usgs_nwis_clean_v2 %>% 
    filter(sta_code == "SDI") %>% 
    pull(df) %>% 
    chuck(1)
...
usgs_nwis_clean_v3 <- usgs_nwis_clean_v2 %>% 
...
usgs_nwis_qc_check <- usgs_nwis_clean_v3 %>% 
  mutate(
    df_count = map(
      df, 
      ~select(.x, -ends_with("_cd")) %>% 
...
First, we need to check if the timestamps of all datetimes of the USGS water quality data are collected on a 15-minute interval (00:00, 15:00, 30:00, and 45:00 for each hour).

```{r look for unrounded timestamps usgs}
# 1) Data downloaded from NWIS:
...
# NWIS data - Unique values for minutes:
usgs_nwis_qc_check$m_summ

# NWIS data - Unique values for seconds:
usgs_nwis_qc_check$s_summ

# 2) Three .xlsx files with non-publicly available USGS data:
# Create lists of unique values for minutes and seconds for each USGS water quality station
usgs_xlsx_qc_check <- usgs_xlsx_clean_v3 %>% 
  mutate(
    df = map(
      df, 
      ~mutate(
...
# non-publicly available data - Unique values for minutes:
usgs_xlsx_qc_check$m_summ

# non-publicly available data - Unique values for seconds:
usgs_xlsx_qc_check$s_summ
```

Some of the data downloaded from NWIS had timestamps with minutes that were not collected on a 15-minute interval (0, 15, 30, 45). All of the timestamps of the three .xlsx files with non-publicly available USGS data were collected at 15-minute intervals. 
...
# NWIS data - Look for duplicated timestamps
usgs_nwis_qc_check
...
# non-publicly available data - Look for duplicated timestamps
usgs_xlsx_qc_check
```

The data downloaded from NWIS for the RVB, LIB, RYI_1, and SDI stations have a few duplicated timestamps after rounding them to the nearest 15-minute interval. This needs to be investigated further. All other data was okay. We'll look at RVB first:

```{r investigate dup timestamp data rvb}
# RVB
usgs_nwis_qc_check$df_dup_join[[3]]
```
...
# LIB
usgs_nwis_qc_check$df_dup_join[[8]]
```
...
# RYI_1
usgs_nwis_qc_check$df_dup_join[[9]]
```
...
# SDI
usgs_nwis_qc_check$df_dup_join[[10]]
```
...
usgs_nwis_clean_v4 <- usgs_nwis_clean_v3 %>% 
  filter(!sta_code %in% c("LIB", "RYI_1", "SDI")) %>% 
  bind_rows(lib_ryi1_sdi_clean)
```
...
Now we can round the timestamps of the USGS water quality data downloaded from NWIS to their nearest 15-minute intervals. The three .xlsx files with non-publicly available USGS data already have rounded timestamps.

```{r round timestamps usgs}
usgs_nwis_clean_v5 <- usgs_nwis_clean_v4 %>% 
  mutate(
    df_dt_round = map(
      df, 
      ~mutate(.x, DateTime = round_date(DateTime, unit = "15 minute"))
...
Next, we need to standardize the parameter names of the USGS water quality data. The three .xlsx files with non-publicly available USGS data already have standardized parameter names.

```{r std parameter names usgs, message = FALSE}
usgs_nwis_clean_v6 <- usgs_nwis_clean_v5 %>% 
  mutate(
...
### Combine NWIS and xlsx data

Now, we need to combine the data downloaded from NWIS and the non-publicly available USGS data for LIB and RYI_1. We will wait to add the flow data from SDI to the water quality data until later in this script.

#### LIB

We will keep all data that was downloaded from NWIS and add the following data from the .xlsx data set:

* Chla: start of data set (7/16/2013) to 10/17/2014 9:15, 9/3/2015 8:15 to 2/4/2018 23:45
* DO: 1/1/2016 00:00 to 2/4/2018 23:45
* fDOM: start of data set (7/16/2013) to 2/3/2018 23:45
* Flow: none of it
* FlowTF: start of data set (10/1/2014) to 2/2/2015 10:00
* NitrateNitrite: start of data set (7/16/2013) to 2/4/2018 23:45
* pH: 10/1/2016 00:00 to 2/3/2018 23:45
* SpCnd: 10/1/2016 00:00 to 2/3/2018 23:45
* Turbidity: none of it
* WaterTemp: 8/4/2015 00:00 to 12/31/2015 23:45, 10/1/2016 00:00 to 9/12/2017 9:00

```{r combine nwis and xlsx lib, message = FALSE}
# Prepare .xlsx data set to be added to the NWIS data set
lib_xlsx_filt <- usgs_xlsx_clean_v3 %>%
  # Pull out non-publicly available data for LIB and pivot to long format for easier filtering and combining
  filter(sta_code == "LIB") %>%
  pull(df) %>%
  chuck(1) %>%
  pivot_longer(
  ...
  filter(!is.na(Value)) %>%
  filter(
    # Remove Flow and Turbidity data
    !Parameter %in% c("Flow", "Turbidity"),
    # Remove the ends of the Chla, DO, and NitrateNitrite data
    !(Parameter %in% c("Chla", "DO", "NitrateNitrite") & DateTime > "2018-02-04 23:45:00"),
    # Remove the ends of the fDOM, pH, and SpCnd data
    !(Parameter %in% c("fDOM", "pH", "SpCnd") & DateTime > "2018-02-03 23:45:00"),
    # Finish filtering Chla data
    !(Parameter == "Chla" & (DateTime > "2014-10-17 09:15:00" & DateTime < "2015-09-03 08:15:00")),
    # Finish filtering DO data
    !(Parameter == "DO" & DateTime < "2016-01-01 00:00:00"),
    # Filter FlowTF data
    !(Parameter == "FlowTF" & DateTime > "2015-02-02 10:00:00"),
    # Finish filtering pH and SpCnd data
    !(Parameter %in% c("pH", "SpCnd") & DateTime < "2016-10-01 00:00:00"),
    # Filter WaterTemp data
    !(Parameter == "WaterTemp" & DateTime < "2015-08-04 00:00:00"),
    !(Parameter == "WaterTemp" & 
        (DateTime > "2015-12-31 23:45:00" & DateTime < "2016-10-01 00:00:00")),
    !(Parameter == "WaterTemp" & DateTime > "2017-09-12 09:00:00")
  )

# Pull out data downloaded from NWIS for LIB
lib_nwis <- usgs_nwis_clean_v6 %>% 
  filter(sta_code == "LIB") %>%
  pull(df) %>%
  chuck(1)

# Add non-publicly available data to NWIS data
lib_nwis_xlsx_dv <- lib_nwis %>% 
  # Pivot data values for the NWIS LIB data to long format
  select(-c(site_no, ends_with("_Qual"))) %>% 
  pivot_longer(
    cols = where(is.numeric),
    names_to = "Parameter",
    values_to = "Value"
  ...
  filter(!is.na(Value)) %>% 
  bind_rows(lib_xlsx_filt) %>% 
  # Pivot data back to wide format
  pivot_wider(names_from = Parameter, values_from = Value)

# Join NWIS quality codes back to data values
lib_nwis_qc <- lib_nwis %>% select(DateTime, ends_with("_Qual"))

lib_f <- 
  left_join(lib_nwis_xlsx_dv, lib_nwis_qc) %>% 
  mutate(site_no = "11455315", .before = DateTime)
```
...
* Chla: all data in data set (2015 and 2016)
* DO: all data in data set (2015 and 2016)
* fDOM: all data in data set (2015 and 2016)
* Flow: none of it
* FlowTF: none of it
* NitrateNitrite: none of it
* pH: all data in data set (2015 and 2016)
* SpCnd: all data in data set (2015 and 2016)
* Turbidity: all data in data set (2015 and 2016)
* WaterTemp: all data in data set (2015 and 2016)

```{r combine nwis and xlsx ryi_1, message = FALSE}
# Prepare .xlsx data set to be added to the NWIS data set
ryi1_xlsx_filt <- usgs_xlsx_clean_v3 %>%
  # Pull out non-publicly available data for RYI_1 and pivot to long format for easier filtering and combining
  filter(sta_code == "RYI") %>%
  pull(df) %>%
  chuck(1) %>%
  pivot_longer(
  ...
  filter(!is.na(Value)) %>%
  # Remove Flow, FlowTF, and NitrateNitrite data
  filter(!Parameter %in% c("Flow", "FlowTF", "NitrateNitrite"))

# Pull out data downloaded from NWIS for RYI_1
ryi1_nwis <- usgs_nwis_clean_v6 %>% 
  filter(sta_code == "RYI_1") %>%
  pull(df) %>%
  chuck(1)

# Add non-publicly available data to NWIS data
ryi1_nwis_xlsx_dv <- ryi1_nwis %>% 
  # Pivot data values for the NWIS RYI_1 data to long format
  select(-c(site_no, ends_with("_Qual"))) %>% 
  pivot_longer(
    cols = where(is.numeric),
    names_to = "Parameter",
  ...
  filter(!is.na(Value)) %>% 
  bind_rows(ryi1_xlsx_filt) %>%
  # Pivot data back to wide format
  pivot_wider(names_from = Parameter, values_from = Value)

# Join NWIS quality codes back to data values; add back site_no to be used later in the script
ryi1_nwis_qc <- ryi1_nwis %>% select(DateTime, ends_with("_Qual"))

ryi1_f <- 
  left_join(ryi1_nwis_xlsx_dv, ryi1_nwis_qc) %>% 
  mutate(site_no = "11455350", .before = DateTime)
```

Add back the combined data for LIB and RYI_1 to the main dataframe for further processing.

```{r add combined lib and ryi data}
lib_ryi1_comb <- tibble(
  sta_code = c("LIB", "RYI_1"),
  df = list(lib_f, ryi1_f)
)

usgs_clean_v1 <- usgs_nwis_clean_v6 %>% 
  filter(!sta_code %in% c("LIB", "RYI_1")) %>% 
  bind_rows(lib_ryi1_comb)
```
...
* fDOM, Flow, FlowTF, pH, DO, SpCnd, Turbidity, and WaterTemp - keep all data from RYI_1 (11455350) through 9/4/2018 (which is the end of the period of record for the RYI_1 water quality parameters). Switch to data from RYI_2 (11455385) starting on 9/5/2018.
* Chla - data for RYI_1 (11455350) appears noisy at the end of its period of record where it overlaps with RYI_2 (11455385). We will switch to using data from RYI_2 (11455385) at the start of its period of record (6/14/2018 12:15), and only use data for RYI_1 (11455350) where there is no overlap in 2018.
...
ryi_c_long_filt <- ryi_c_long %>% 
  ...
  filter(!is.na(value)) %>% 
  # Trim RYI_1 and RYI_2 data so that they don't overlap with 9/5/2018 as the cutoff date, except for Chla and NitrateNitrite
  filter(!(
    site_no == "11455350" & 
...
### Add SDI flow data
Now, we will add the flow data from SDI that was provided by Jared Frantzich to the water quality data.

```{r add sdi flow data, message = FALSE}
# Pull out WQ data for SDI
sdi_wq <- usgs_clean_v2 %>% 
  filter(sta_code == "SDI") %>% 
  pull(df) %>% 
  chuck(1)

# Pull out flow data for SDI
sdi_flow <- usgs_xlsx_clean_v3 %>% 
  filter(sta_code == "SDI") %>% 
  pull(df) %>% 
  chuck(1)

# Join flow data to the WQ data
sdi_wq_flow_c <- full_join(sdi_wq, sdi_flow)
