...
sdi_flow_orig <- read_excel(file.path(fp_abs_wq_raw, "RTM_RAW_USGS_SDI Flow_2011-2018.xlsx"))
```
...
usgs_clean_v1 <- usgs_dt_clean %>% 
  mutate(
    df = map(df, ~filter(.x, !if_all(where(is.numeric), is.na)))
  )

sdi_flow_clean_v1 <- sdi_flow_clean %>% filter(!if_all(where(is.numeric), is.na))
```
