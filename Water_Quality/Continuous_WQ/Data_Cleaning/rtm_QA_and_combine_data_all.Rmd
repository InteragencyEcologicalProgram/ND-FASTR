...
We will first flag and exclude suspect data for individual stations and parameters through visual inspection of the plots of raw data and through information from the field and sonde verification records.
...
The chlorophyll data is very noisy and suspect at LIB from 7/13/2018 to 8/14/2018. We will flag and exclude this data.
...
```{r rm lib chla 2018}
# Flag and exclude chlorophyll values in 2018
...
There are a few negative chlorophyll values in the SRH data set in Oct-Nov 2011 and Aug 2014 that need to be flagged and excluded.

```{r plot srh chla oct nov 2011 before, message = FALSE}
...
```{r rm srh chla 2011 and 2014}
# Flag and exclude negative chlorophyll values in Oct-Nov 2011 and Aug 2014
...
The negative chlorophyll values are excluded from the data set now:
```{r plot srh chla oct nov 2011 after, message = FALSE}
...
```{r plot srh chla aug 2014 after, message = FALSE}
...
mod_stations <- c("I80", "LIB", "LIBCUT", "LIS", "RCS", "SDI", "SGG", "SRH", "STTD")
...
rtm_wq_proc_clean_v1 %>% 
  ...
  write_excel_csv(file.path(fp_abs_wq_qa_f, "RTM_INPUT_all_2021-02-19.csv"), na = "")
```
