...
We need to look for any duplicated timestamps after rounding them to the nearest 15-minute interval.
...
The LIB station has 7 duplicated timestamps after rounding them to the nearest 15-minute interval. This needs to be investigated further.
...
There were extra Chla values recorded on:
...
All USGS data is now ready to have the timestamps rounded to their nearest 15-minute intervals and the parameter names to be standardized.
...
lis_rcs_flow_orig <- wqes_cont_files_flow %>% 
  mutate(df_flow = map2(filepath, sta_code, .f = import_wqes_data_flow)) %>% 
...
All DWR-NRCO-WQES data is now ready to have the timestamps rounded to their nearest 15-minute intervals and the parameter names to be standardized.
