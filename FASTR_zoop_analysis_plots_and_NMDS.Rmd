...
zoop <- read.csv("LT_RAW_ZoopThru2018_20200825.csv", header = TRUE)
...
zoop_2019 <- read.csv("NDFA_2019_zoop_CPUE_allsites_alldates.csv", header = TRUE, stringsAsFactors = FALSE)
...
zoop_taxa <- read.csv("YB_TaxonomyTable.csv", header = TRUE, stringsAsFactors = FALSE)
...
#read in file
WQ_2019 <- read.csv("2019_NDFA_physical data.csv", stringsAsFactors = FALSE)
...
#changed dates to match Cat's FlowDatesDesignations file
#2011 
...
#2014
zoop_2014 <- zoop[zoop$Date>="2014-08-09" & zoop$Date<="2014-10-24", ]
#2015
zoop_2015 <- zoop[zoop$Date>="2015-07-21" & zoop$Date<="2015-11-01", ]
#2016
zoop_2016 <- zoop[zoop$Date>="2016-06-13" & zoop$Date<="2016-09-01", ]
#2017
zoop_2017 <- zoop[zoop$Date>="2017-07-29" & zoop$Date<="2017-10-19", ]
#2018
zoop_2018 <- zoop[zoop$Date>="2018-07-28" & zoop$Date<="2018-10-27", ]
...
#adding sampling period using Cat's FlowDatesDesignations file. Using Before, During, After designations
#read in file
FlowDesignation <- read.csv("FlowDatesDesignations.csv")

#make Date columns a date
FlowDesignation$PreFlowStart <- format(as.Date(FlowDesignation$PreFlowStart, format = "%m/%d/%Y"), "%Y-%m-%d")

FlowDesignation$PreFlowEnd <- format(as.Date(FlowDesignation$PreFlowEnd, format = "%m/%d/%Y"), "%Y-%m-%d")

FlowDesignation$PostFlowStart <- format(as.Date(FlowDesignation$PostFlowStart, format = "%m/%d/%Y"), "%Y-%m-%d")

FlowDesignation$PostFlowEnd <- format(as.Date(FlowDesignation$PostFlowEnd, format = "%m/%d/%Y"), "%Y-%m-%d")

#Set dates in FlowDesignation as characters
FlowDesignation$PreFlowStart <- as.character(FlowDesignation$PreFlowStart)
FlowDesignation$PreFlowEnd <- as.character(FlowDesignation$PreFlowEnd)
FlowDesignation$PostFlowStart <- as.character(FlowDesignation$PostFlowStart)
FlowDesignation$PostFlowEnd <- as.character(FlowDesignation$PostFlowEnd)
FlowDesignation$Year <- as.character(FlowDesignation$Year)

#add in columns from FlowDesignation file
zoop_NDFA <- inner_join(zoop_NDFA,FlowDesignation, by = "Year")

#add in Before, During, After
zoop_NDFA <- zoop_NDFA %>%
   mutate(SamplePeriod = ifelse(Date > PreFlowStart & Date < PreFlowEnd, "Before", NA)) %>%
   mutate(SamplePeriod = replace(SamplePeriod, Date > PreFlowEnd & Date < PostFlowStart, "During")) %>% 
   mutate(SamplePeriod = replace(SamplePeriod, Date > PostFlowStart & Date < PostFlowEnd, "After")) %>%
  filter(!is.na(SamplePeriod)) %>%
...
#create heat map using total CPUE summed by sampling period (B, D, A) and year
###NEED TO CREATE SAMPLEPERIOD CODE!!!###
zoop_NDFA_sum_year_period <- zoop_NDFA %>% 
  group_by(Year, SamplePeriod) %>% summarize(CPUEZoop = sum(CPUEZoop))

yearperiodplot <- ggplot(zoop_NDFA_sum_year_period, aes(x = SamplePeriod, y = Year, fill = CPUEZoop)) +
  geom_tile() +
  scale_fill_distiller(palette = "YlOrRd", trans = "reverse") +
...
###subsetting by classification

#sum by Classificaiton
zoop_NDFA_sum_year_period_class <- zoop_NDFA %>% 
  group_by(Year, SamplePeriod, Classification) %>% summarize(CPUEZoop = sum(CPUEZoop))

#calanoids
yearperiodplot_cal <- ggplot(subset(zoop_NDFA_sum_year_period_class, Classification %in% "Calanoids"), aes(x = SamplePeriod, y = Year, fill = CPUEZoop)) +
  geom_tile() +
  scale_fill_distiller(palette = "YlOrRd", trans = "reverse") +
  scale_x_discrete(limits = c("Before", "During", "After"))
...
#cyclopoids
yearperiodplot_cyclo <- ggplot(subset(zoop_NDFA_sum_year_period_class, Classification %in% "Cyclopoids"), aes(x = SamplePeriod, y = Year, fill = CPUEZoop)) +
  geom_tile() +
  scale_fill_distiller(palette = "YlOrRd", trans = "reverse") +
  scale_x_discrete(limits = c("Before", "During", "After"))

yearperiodplot_cyclo

#cladocera
yearperiodplot_cla <- ggplot(subset(zoop_NDFA_sum_year_period_class, Classification %in% "Cladocera"), aes(x = SamplePeriod, y = Year, fill = CPUEZoop)) +
  geom_tile() +
  scale_fill_distiller(palette = "YlOrRd", trans = "reverse") +
  scale_x_discrete(limits = c("Before", "During", "After"))
...
#microzoop
yearperiodplot_micro <- ggplot(subset(zoop_NDFA_sum_year_period_class, Classification %in% "Microzooplankton_Nauplii"), aes(x = SamplePeriod, y = Year, fill = CPUEZoop)) +
  geom_tile() +
  scale_fill_distiller(palette = "YlOrRd", trans = "reverse") +
  scale_x_discrete(limits = c("Before", "During", "After"))

yearperiodplot_micro
...
#remove duplicate for SHR that was found when trying to do NMDS but wouldn't allow data to spread
#SHR 9/14/15 Calanoid Copepodid
###this is the NMDS file that you can subset to get different combos###
zoop_NDFA_NMDS <- zoop_NDFA_NMDS[-c(1637),]

#create sitedate column- no longer needed
...
###subset 1 using organism###
#Faster way:
zoop_NDFA_NMDS1 = select(zoop_NDFA_NMDS, Date, StationCode, Organism, CPUEZoop)
...
all.flip1 <- spread(data=zoop_NDFA_NMDS1, key = Organism, value= CPUEZoop, fill = 0)
...
ZoopCPUE1 = all.flip1[3:86]

#change to matrix for vegan
m.ZoopCPUE1 <- as.matrix(ZoopCPUE1)
...
nmds1 <- metaMDS(m.ZoopCPUE1, distance = "bray", trymax = 500)
...
###subset 2 using classification###
zoop_NDFA_NMDS2 <- select(zoop_NDFA_NMDS, Date, Month, Year, StationCode, Classification, CPUEZoop)
...
#Here, Rosie is assigning colors to each site and calling the column name col
...
#then she merges them with the spread data, with classification as the column headers
all.flip2 = merge(all.flip2, stationColors)

#make year colors
YearColors = data.frame(Year = unique(all.flip2$Year),
                           Ycol = c("#8DD3C7", "#FFFFB3", "#BEBADA",
                                   "#FB8072", "#80B1D3", "#FDB462",
                                   "#B3DE69", "#FCCDE5", "#D9D9D9"))

#then merge them with the spread data, with classification as the column headers
all.flip2 = merge(all.flip2, YearColors)
...
orditorp(nmds2, display="sites", col=all.flip2$col,
         air=0.01)
#color points by year. not sure how to pick year, so said to display "sites"
orditorp(nmds2, display="sites", col=all.flip2$Ycol,
         air=0.01)
#add classification names
orditorp(nmds2, display="species", col="red", air=0.01)


###subset 3 looking at genus and CPUE###
zoop_NDFA_NMDS3 <- select(zoop_NDFA_NMDS, Date, Year, StationCode, Genus, CPUEZoop)

#spread the data so that Genus are the column headers, CPUE is the cell value, and sitedate is the row names. the values_fn sums if there are the same genus on the same date and site. 
all.flip3 <- pivot_wider(data=zoop_NDFA_NMDS3, names_from = Genus, values_from = CPUEZoop, values_fill = 0, values_fn = sum)

#extract columns with abundance info
ZoopCPUE3 = as.matrix(all.flip3[4:51])

nmds3 <- metaMDS(ZoopCPUE3, distance = "bray", trymax = 500)

#merge station colors with all.flip3
all.flip3 = merge(all.flip3, stationColors)

#merge year colors with all.flp3
all.flip3 = merge(all.flip3, YearColors)

plot(nmds3)

ordiplot(nmds3, type="n")
ordihull(nmds3, groups=all.flip3$Year, draw="polygon", col="grey90", label=F)
#color points by site
orditorp(nmds3, display="sites", col=all.flip3$col,
         air=0.01)
#color points by year. not sure how to pick year, so said to display "sites"
orditorp(nmds3, display="sites", col=all.flip3$Ycol,
         air=0.01)
#add classification names
orditorp(nmds3, display="species", col="red", air=0.01)

#no structure using Genus. nMDS didn't converge



#trying with summed CPUE by region and classification
#create region column and create new table with summation by region

#create copy of station column
zoop_NDFA_NMDS$Region=zoop_NDFA_NMDS$StationCode

#rename stations to regions

zoop_NDFA_NMDS$Region[zoop_NDFA_NMDS$Region =="RMB"] <- "Colusa Drain/Ridge Cut"
zoop_NDFA_NMDS$Region[zoop_NDFA_NMDS$Region =="RCS"] <- "Colusa Drain/Ridge Cut"
zoop_NDFA_NMDS$Region[zoop_NDFA_NMDS$Region =="RD22"] <- "Central Yolo Bypass"
zoop_NDFA_NMDS$Region[zoop_NDFA_NMDS$Region =="I80"] <- "Central Yolo Bypass"
zoop_NDFA_NMDS$Region[zoop_NDFA_NMDS$Region =="LIS"] <- "Lower Yolo Bypass"
zoop_NDFA_NMDS$Region[zoop_NDFA_NMDS$Region =="STTD"] <- "Lower Yolo Bypass"
zoop_NDFA_NMDS$Region[zoop_NDFA_NMDS$Region =="BL5"] <- "Cache Slough Complex"
zoop_NDFA_NMDS$Region[zoop_NDFA_NMDS$Region =="LIB"] <- "Cache Slough Complex"
zoop_NDFA_NMDS$Region[zoop_NDFA_NMDS$Region =="RYI"] <- "Cache Slough Complex"
zoop_NDFA_NMDS$Region[zoop_NDFA_NMDS$Region =="SDI"] <- "Lower Sac River"
zoop_NDFA_NMDS$Region[zoop_NDFA_NMDS$Region =="RVB"] <- "Lower Sac River"

#sum total CPUE by region and period
zoop_NDFA_NMDS_region_class_sum <- zoop_NDFA_NMDS %>% 
  group_by(Date, Region, Classification, Year) %>% summarize(CPUEZoop = sum(CPUEZoop))

#run nMDS using new summation by region, date, and classification
all.flip4 <- pivot_wider(data=zoop_NDFA_NMDS_region_class_sum, names_from = Classification, values_from = CPUEZoop, values_fill = 0, values_fn = sum)

#extract columns with abundance info
ZoopCPUE4 = as.matrix(all.flip4[4:9])

nmds4 <- metaMDS(ZoopCPUE4, distance = "bray", trymax = 500)


regionColors = data.frame(Region = unique(all.flip4$Region),
                           col = c("#8DD3C7", "#FFFFB3", "#BEBADA",
                                   "#FB8072", "#80B1D3", "#FDB462"))

#merge station colors with all.flip4
all.flip4 = merge(all.flip4, regionColors)

#merge station years with all.flip4
all.flip4 = merge(all.flip4, YearColors)

plot(nmds4)

ordiplot(nmds4, type="n")
ordihull(nmds4, groups=all.flip4$Year, draw="polygon", col="grey90", label=F)
#color points by region
orditorp(nmds4, display="sites", col=all.flip4$col,
         air=0.01)
#color points by year. not sure how to pick year, so said to display "sites"
orditorp(nmds4, display="sites", col=all.flip4$Ycol,
         air=0.01)
#add classification names
orditorp(nmds4, display="species", col="red", air=0.01)

#nMDS4 has some structure and was solved very fast. ~40 iterations
