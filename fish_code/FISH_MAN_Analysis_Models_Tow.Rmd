...
Mixed modeling, including Station as random effect
SLOW!

```{r}
f7 <- formula(sumCount ~ Region + fYear + ActionPhase + offset(Samp)+ (1|StationCode))
library(optimx)
mnb1 <- glmer.nb(f7, data = Tow_smeltish, control = glmerControl(optimizer= "optimx",
...
```{r}
m_means1 <- emmeans(zinb1, ~fYear, adjust = "sidak")
m_means1
...
Post-hoc
```{r}
m_means2 <- emmeans(zinb2, ~fYear, adjust = "sidak")
...
# Proportion Species
ggplot(Tow_f, aes(x = fYear, y = CPUE, fill = CommonName)) + 
  geom_col() + 
...
```{r}
m_means3 <- emmeans(m5all, ~Region|fYear, adjust = "sidak")
