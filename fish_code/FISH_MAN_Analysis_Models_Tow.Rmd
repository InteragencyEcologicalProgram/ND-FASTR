...
Fish_all1 <- left_join(Fish_all0, Regions_min2, by = c("StationCode", "Survey"))
...
# Code to change StationCode name
Fish_all <- Fish_all %>%
  rename(StationCodeInitial = StationCode,
...
Tow_smeltish <- Tow_smeltish0 %>%
  group_by(Survey, fYear, StationCode, ActionPhase, Region, Date,  VolumeSampled, Tow) %>%
  summarize(sumCPUE = sum(CPUE),
            sumCount = sum(sum.count))


Tow_smeltish$ActionPhase <-  factor(Tow_smeltish$ActionPhase, levels(Tow_smeltish$ActionPhase)[c(2,3,1)])
```
...
# Action Phase
SmeltishAP <- ggplot(Tow_smeltish, aes(x = ActionPhase, y = sumCPUE)) + geom_jitter(aes(color = Region), size = 3) + scale_color_viridis(discrete = TRUE, option = "viridis") + theme_bw()
...
ggplot(Tow_summary_yr_smeltish_1, aes(x = fYear, y = meanCPUE, ymin = meanCPUE - seCPUE, ymax = meanCPUE + seCPUE, fill = ActionPhase)) + 
...
#### Barplots for figures - ActionPhase/Region and yr/Region
...
plot4 <- ggplot(Tow_summary_smeltish, aes(x = ActionPhase, y = meanCPUE, ymin = meanCPUE - seCPUE, ymax = meanCPUE + seCPUE, fill = Region)) + 
  geom_bar(position = position_dodge(width = 0.9), stat = "identity") + 
  geom_errorbar(position = position_dodge(width = 0.9), width = 0.4) + 
  scale_fill_viridis(discrete = TRUE, option = "viridis") + theme_bw() + 
    annotate("text", x = "Post", y = .0055, label = "D", size = 8)  + 
  theme(axis.text = element_text(size = 14),
        axis.title = element_text(size = 15),
        legend.text = element_text(size = 14),
        legend.title = element_text(size = 15))
...
Tow_smeltish2$ActionPhase <-  factor(Tow_smeltish2$ActionPhase, levels(Tow_smeltish2$ActionPhase)[c(2,3,1)])

```

Replot data - outliers gone
```{r}
ggplot(Tow_smeltish2, aes(fYear, meanCPUE, fill = ActionPhase)) + geom_boxplot() +
    scale_fill_viridis(discrete = TRUE, option = "viridis") + theme_bw() + 
  theme(axis.text = element_text(size = 14),
        axis.title = element_text(size = 15),
        legend.text = element_text(size = 14),
...
Run models
```{r}
(M.smeltish1 <- glm(meanCPUE~Region + fYear + ActionPhase, data  = Tow_smeltish2))
summary(M.smeltish1)

(M.smeltish2 <- glm(meanCPUE~Region + fYear * ActionPhase, data  = Tow_smeltish2))
summary(M.smeltish2)

(M.smeltish3 <- glm(meanCPUE~Region * fYear + ActionPhase, data  = Tow_smeltish2))
summary(M.smeltish3)

(M.smeltish4 <- glm(meanCPUE~Region * ActionPhase + fYear, data  = Tow_smeltish2))
summary(M.smeltish4)
...
```{r}
library(car)
Anova(M.smeltish3, type = 3)
```

Post-hoc
```{r}

emmip(M.smeltish3, fYear~ActionPhase|Region)
emmip(M.smeltish3, Region~ActionPhase|fYear)

m_means1a <- emmeans(M.smeltish3, pairwise~fYear|Region, adjust="sidak")
m_means1a

m_means1b <- emmeans(M.smeltish3, pairwise~Region|fYear, adjust="sidak")
m_means1b

m_means1c <- emmeans(M.smeltish3, pairwise~ActionPhase, adjust="sidak")
m_means1c
```
...
Tow_natives$ActionPhase <-  factor(Tow_natives$ActionPhase, levels(Tow_natives$ActionPhase)[c(2,3,1)])
```
...
# Year 
ggplot(Tow_natives, aes(x = ActionPhase, y = sumCPUE)) + geom_jitter(aes(color = Region), size = 3) + facet_wrap(~fYear) + scale_color_viridis(discrete = TRUE, option = "magma") + theme_bw()
...
ggplot(Tow_summary_native_yr3, aes(x = fYear, y = meanCPUE, ymin = meanCPUE - seCPUE, ymax = meanCPUE + seCPUE, fill = ActionPhase)) + 
  geom_bar(position = position_dodge(width = 0.9), stat = "identity") + 
  geom_errorbar(position = position_dodge(width = 0.9), width = 0.4) + 
  scale_fill_viridis(discrete = TRUE, option = "viridis") + theme_bw() + 
  theme(axis.text = element_text(size = 14),
...
plot5 <- ggplot(Tow_summary_native2, aes(x = ActionPhase, y = meanCPUE, ymin = meanCPUE - seCPUE, ymax = meanCPUE + seCPUE, fill = Region)) + 
  geom_bar(position = position_dodge(width = 0.9), stat = "identity") + 
  geom_errorbar(position = position_dodge(width = 0.9), width = 0.4) + 
  scale_fill_viridis(discrete = TRUE, option = "viridis") + 
      annotate("text", x = "Post", y = 3e-05, label = "E", size = 8)  + 
  theme_bw() + 
  theme(axis.text = element_text(size = 14),
        axis.title = element_text(size = 15),
        legend.text = element_text(size = 14),
...
Tow_natives2$ActionPhase <-  factor(Tow_natives2$ActionPhase, levels(Tow_natives2$ActionPhase)[c(2,3,1)])

```

Run models
```{r}
(M.natives1 <- glm(meanCPUE~Region + fYear + ActionPhase, data  = Tow_natives2))
summary(M.natives1)

(M.natives2 <- glm(meanCPUE~Region + fYear * ActionPhase, data  = Tow_natives2))
summary(M.natives2)

(M.natives3 <- glm(meanCPUE~Region * fYear + ActionPhase, data  = Tow_natives2))
summary(M.natives3)

(M.natives4 <- glm(meanCPUE~Region * ActionPhase + fYear, data  = Tow_natives2))
summary(M.natives4)
...
Tow_all$ActionPhase <-  factor(Tow_all$ActionPhase, levels(Tow_all$ActionPhase)[c(2,3,1)])
```
...
# Year 
ggplot(Tow_all, aes(x = ActionPhase, y = sumCPUE)) + geom_jitter(aes(color = Region), size = 3) + facet_wrap(~fYear) + scale_color_viridis(discrete = TRUE, option = "magma") + theme_bw()
...
ggplot(Tow_summary__all_yr3, aes(x = fYear, y = meanCPUE, ymin = meanCPUE - seCPUE, ymax = meanCPUE + seCPUE, fill = ActionPhase)) + 
  geom_bar(position = position_dodge(width = 0.9), stat = "identity") + 
  geom_errorbar(position = position_dodge(width = 0.9), width = 0.4) + 
  scale_fill_viridis(discrete = TRUE, option = "viridis") + theme_bw() + 
  theme(axis.text = element_text(size = 14),
...
plot6 <- ggplot(Tow_summary_all3, aes(x = ActionPhase, y = meanCPUE, ymin = meanCPUE - seCPUE, ymax = meanCPUE + seCPUE, fill = Region)) + 
  geom_bar(position = position_dodge(width = 0.9), stat = "identity") + 
  geom_errorbar(position = position_dodge(width = 0.9), width = 0.4) + 
  scale_fill_viridis(discrete = TRUE, option = "viridis") + 
      annotate("text", x = "Post", y = .07, label = "F", size = 8)  + 
  theme_bw() + 
  theme(axis.text = element_text(size = 14),
        axis.title = element_text(size = 15),
        legend.text = element_text(size = 14),
...
f6 <- formula(sumCount~ Region + ActionPhase + fYear + Region:ActionPhase + ActionPhase:fYear + offset(Samp) + (1|StationCode))
...
Post-hoc
```{r}
towallcontrast <- emmeans(M.glmer5b, ~ActionPhase * fYear * Region)
...
Tow_all2$ActionPhase <-  factor(Tow_all2$ActionPhase, levels(Tow_all2$ActionPhase)[c(2,3,1)])

```

Run models
```{r}
(M.all1 <- glm(meanCPUE~Region + fYear + ActionPhase, data  = Tow_all2))
summary(M.all1)

(M.all2 <- glm(meanCPUE~Region + fYear * ActionPhase, data  = Tow_all2))
summary(M.all2)

(M.all3 <- glm(meanCPUE~Region * fYear + ActionPhase, data  = Tow_all2))
summary(M.all3)

(M.all4 <- glm(meanCPUE~Region * ActionPhase + fYear, data  = Tow_all2))
summary(M.all4)
