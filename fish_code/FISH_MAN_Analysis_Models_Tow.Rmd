...
```{r}
ggplot(Tow_smeltish, aes(x = ActionPhase, y = CPUE)) + geom_jitter(aes(color = Region), size = 3) + theme_bw()

ggplot(Tow_smeltish, aes(x = ActionPhase, y = CPUE)) + geom_jitter(aes(color = Region), size = 3) + facet_wrap(~fYear) + theme_bw()
```
...
var(Tow_smeltish$sum.count)
mean(Tow_smeltish$sum.count)
...
* More smeltish in phase "pre" followed by "post" followed by "during"
* More smeltish upstream, then downstream then Middle Sac River (much less)
* Variability by year but nothing too big
* No effect of region or year on the extra zeroes
* More zeroes in during then post then pre

### Native fishes

```{r}
ggplot(Tow_natives, aes(x = ActionPhase, y = CPUE)) + geom_jitter(aes(color = Region), size = 3) + theme_bw()

ggplot(Tow_natives, aes(x = ActionPhase, y = CPUE)) + geom_jitter(aes(color = Region), size = 3) + facet_wrap(~fYear) + theme_bw()
```
...
Zero-inflated model
Could probably try a normal negative binomial as well
```{r}
...
plot(x=Tow_f$fYear, y = EP3, main = "Year")
