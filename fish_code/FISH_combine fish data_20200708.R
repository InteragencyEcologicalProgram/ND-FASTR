...
#EDSM - kodiak trawl
#DJFMP: beach seine
#Yolo: fyke, rotary screww trap, beach seine
#add location infomrmation for stations that don't have any

stations = read.csv("FISH_MAN_AllIEPstations_20200527.csv", stringsAsFactors = F)
...
FMWT2 = pivot_longer(FMWT, 
                     cols = `Aequorea spp (Lens Jellyfish)`:`Yellowfin Goby`, #These are the columns we want to pivot
...
# Obtained from Catherine Johnston at USFWS
library(readr)
EDSM_KDTR <- read_csv("fish data/2020-04-09_EDSM_all data through Nov 2019.csv", 
                      col_types = cols( 
                                       Time = col_time(format = "%H:%M:%S"),
                                       SpecialStudy = col_factor(),
                                       SpecialStudyID = col_character(),
                                       Tide = col_factor(),
                                       StartDepth = col_number()))
...
EDSM = EDSM[,c(2,3,4,5,6,7,8,9,10, 12, 13,18,20,21:31,33, 34)]

# define date
EDSM$Date <- as.Date(EDSM$Date, format = "%m/%d/%Y")
...
EDSM <- EDSM %>%
  rename(
    ...
    Turbidity = Turb,
    Secchi = Scchi,
    VolumeSampled = Volume,
    DJFMP = CommonName,
    Count = SumOfCatchCount)
                
#add something so you can tell it's EDSM
EDSM$Survey = "EDSM"

#Yolo bypass
# Obtained from Catarina Pien at DWR
YBFMP <- read_csv("fish data/FISH_RAW_YBFMP_WQ_Fish_2011_thru_2019_20200319.csv", 
...
#rename a few of the columns to make it easier to join with the other data sets
...
#fix beach seines labeled as "PCS"
YBFMP2$StationCode[which(YBFMP2$MethodCode == "BSEIN" & YBFMP2$StationCode == "PCS")] = "BL1"

#add fyke effort hours 
#fyke effort data obtained from Nicole Kwan at DWR; calculations & QC for effort can be found in "Add Fyke Effort.rmd"; 
fyke <- read_csv("YBFMP.fyke.hours.csv")
YBFMP3 = left_join(YBFMP2, fyke, by = c("DateTime", "StationCode"))
names(YBFMP3)[35] <- "HoursSampled"
...
# Original data obtained at: https://portal.edirepository.org/nis/mapbrowse?packageid=edi.244.4
DJFMP_BS <- read_csv("fish data/edi.244.4/1976-2019_DJFMP_beach_seine_fish_and_water_quality_data.csv", 
                     col_types = cols(AlternateSite = col_character(),Conductivity = col_number(), 
                                      ...
                                      Volume = col_number(),
                                      SeineLength = col_number(),
                                      SeineWidth = col_number(),
                                      SiteDisturbance = col_character(),
                                      StageCode = col_factor(),
                                      Maturation = col_factor()))
...
fishnamescrosswalk <- read_excel("FISH_MAN_namescrosswalk_20200319.xlsx")
...
FMWT4 = mutate(FMWT3, Tow = 1, DO = NA) %>%
  select(Date, Survey, StationCode, MethodCode, Count,
         Secchi, Conductivity, WaterTemp, Turbidity, DO,
         CommonName, Year, Tow, Depth, VolumeSampled, Latitude, Longitude)

#add standardized names to Yolo
Yolo = merge(YBFMP3, unique(fishnamescrosswalk[,c(1,2)]))
...
Yolo2 = mutate(Yolo, Year = year(Date), 
  ...
  select(Date, Survey, StationCode, MethodCode, Count,
         Secchi, Conductivity, WaterTemp, Turbidity, DO,
         CommonName, Year, Tow, Depth, VolumeSampled, HoursSampled, Latitude, Longitude)
...
#Use "bind_rows" to put all the data sets together.
Allfishdata = bind_rows(Yolo2, EDSM3, Townet2, FMWT4, DJFMP3)
...
AllfishdataSum = group_by(Allfishdata, Date, Survey, StationCode, 
                          MethodCode, Secchi, Conductivity, Turbidity, DO,
                          WaterTemp, CommonName, Year, Tow, 
                          Depth, VolumeSampled, HoursSampled, Latitude, Longitude) %>%
  summarize(totalCount = sum(Count))

#Export the final, integrated data set.
today = format(today(), "%Y%m%d")
write.csv(AllfishdataSum, paste0("R_write/FISH_MAN_allIEPsurveys_", today, ".csv"), row.names = F)
