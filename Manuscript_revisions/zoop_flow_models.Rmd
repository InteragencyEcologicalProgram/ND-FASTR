...
ndfa_zoop <- 
  tibble(
    ...
    df_data = list(ls_zoop$RD22, ls_zoop$STTD, ls_zoop$LIB, ls_zoop$RVB)
  ) %>% 
...
ggsave(
  file.path(fp_figures, "zoop_flow_eff_plot_by_year_each_sta.jpg"),
...
ggsave(
  file.path(fp_figures, "zoop_flow_eff_plot_by_year_each_sta.pdf"),
  plot = plt_gam_eff_flow,
  width = 7,
  height = 9,
  units = "in"
...
ggsave(
  file.path(fp_figures, "zoop_year_contrasts_each_sta.jpg"),
...
ggsave(
  file.path(fp_figures, "zoop_year_contrasts_each_sta.pdf"),
  plot = plt_gam_cont_yr,
  width = 6.5,
  height = 5,
  units = "in"
...
ggsave(
  file.path(fp_figures, "zoop_smooth_week.jpg"),
  plot = plt_gam_s_week,
  width = 6.5,
  height = 5,
  units = "in"
...
ggsave(
  file.path(fp_figures, "zoop_smooth_week.pdf"),
  plot = plt_gam_s_week,
  width = 6.5,
  height = 5,
  units = "in"
...
df_gam_anova_tbl_c %>% 
  ...
  write_csv(file.path(fp_tables, "zoop_GAM_anova.csv"))
...
df_gam_eff_flow_stat %>% 
  ...
  write_csv(file.path(fp_tables, "zoop_GAM_eff_flow_stat.csv"))

# Export pairwise Tukey post-hoc year contrasts
df_gam_cont_yr_stat %>% 
  mutate(across(c("Estimate", "SE", "df", "t_ratio", "p_value"), ~ paste0(.x, "##"))) %>% 
  write_csv(file.path(fp_tables, "zoop_GAM_year_contrasts_stat.csv"))
```