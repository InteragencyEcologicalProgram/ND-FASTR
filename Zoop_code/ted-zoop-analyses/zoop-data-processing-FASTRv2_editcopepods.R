...
df_zoop_gen <- df_zoop_gen %>% 
  filter(TaxonName != "Cyclopoida")

df_zoop_gen <- df_zoop_gen %>% 
  filter(TaxonName != "Calanoida")
...
ggsave(path = output,
       filename = "p_zoop_NMDS_region_no_copes_order.png", 
       device = "png",
       scale=1.0, 
       units="in",
       height=5,
...
ggsave(path = output,
       filename = "p_zoop_NMDS_sampleperiod_no_copes_order.png", 
       device = "png",
       scale=1.0, 
       units="in",
       height=5,
...
ggsave(path = output,
       filename = "p_zoop_NMDS_sampleperiod_Region_no_copes_order.png", 
       device = "png",
       scale=1.0, 
       units="in",
       height=5,
...
adon.results <- adonis2(genw2[c(9:68)] ~ genw2$Region*genw2$SamplePeriod + genw2$Year*genw2$SamplePeriod+genw2$Region*genw2$Year, 
                        strata = genw2$StationCode,
                        method = "bray",
                        perm = 999)
#write.csv(adon.results, file = "adon.results.csv")

dm_zoop_gen <- vegdist(genw2[c(9:68)], method = "bray")
