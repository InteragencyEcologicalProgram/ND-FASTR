...
```{r lm flow appl zoop log shr summ, warning = FALSE}
summary(m_zoop_q_appl_shr_log)

visreg(m_zoop_q_appl_shr_log, xvar = "DailyAvgNetFlow", gg = TRUE) + 
  theme_bw() + 
  geom_point(size = 1)

visreg(m_zoop_q_appl_shr_log, xvar = "TotalApplStd", gg = TRUE) + 
  theme_bw() + 
  geom_point(size = 1)
```
...
```{r lm flow appl prior zoop log shr summ, warning = FALSE}
summary(m_zoop_q_appl_pr_shr_log)

visreg(m_zoop_q_appl_pr_shr_log, xvar = "DailyAvgNetFlow", gg = TRUE) + 
  theme_bw() + 
  geom_point(size = 1)

visreg(m_zoop_q_appl_pr_shr_log, xvar = "TotalApplStd_lag1", gg = TRUE) + 
  theme_bw() + 
  geom_point(size = 1)
```
...
```{r lm flow appl zoop sqrt sttd summ, warning = FALSE}
summary(m_zoop_q_appl_sttd_sqrt)

visreg(m_zoop_q_appl_sttd_sqrt, xvar = "DailyAvgNetFlow", gg = TRUE) + 
  theme_bw() + 
  geom_point(size = 1)

visreg(m_zoop_q_appl_sttd_sqrt, xvar = "TotalApplStd", gg = TRUE) + 
  theme_bw() + 
  geom_point(size = 1)
```
...
```{r lm flow appl prior zoop sqrt sttd summ, warning = FALSE}
summary(m_zoop_q_appl_pr_sttd_sqrt)

visreg(m_zoop_q_appl_pr_sttd_sqrt, xvar = "DailyAvgNetFlow", gg = TRUE) + 
  theme_bw() + 
  geom_point(size = 1)

visreg(m_zoop_q_appl_pr_sttd_sqrt, xvar = "TotalApplStd_lag1", gg = TRUE) + 
  theme_bw() + 
  geom_point(size = 1)
```
...
```{r lm flow appl water log shr summ, warning = FALSE}
summary(m_water_q_appl_shr_log)

visreg(m_water_q_appl_shr_log, xvar = "DailyAvgNetFlow", gg = TRUE) + 
  theme_bw() + 
  geom_point(size = 1)

visreg(m_water_q_appl_shr_log, xvar = "TotalApplStd", gg = TRUE) + 
  theme_bw() + 
  geom_point(size = 1)
```
...
```{r lm flow appl prior water shr summ, warning = FALSE}
summary(m_water_q_appl_pr_shr)

visreg(m_water_q_appl_pr_shr, xvar = "DailyAvgNetFlow", gg = TRUE) + 
  theme_bw() + 
  geom_point(size = 1)

visreg(m_water_q_appl_pr_shr, xvar = "TotalApplStd_lag1", gg = TRUE) +
  theme_bw() + 
  geom_point(size = 1)
```
...
```{r lm flow appl prior water log shr summ, warning = FALSE}
summary(m_water_q_appl_pr_shr_log)

visreg(m_water_q_appl_pr_shr_log, xvar = "DailyAvgNetFlow", gg = TRUE) + 
  theme_bw() + 
  geom_point(size = 1)

visreg(m_water_q_appl_pr_shr_log, xvar = "TotalApplStd_lag1", gg = TRUE) + 
  theme_bw() + 
  geom_point(size = 1)
```
...
Both AIC and BIC now prefer the model with the prior month's pesticide application, but the different isn't as large. Since the diagnostics looked better for the model with the prior month's pesticide application using the original values, let's use the prior month model with original values for the model selection procedure. In this case, the best model is the one with the current month's pesticide application.
...
```{r lm flow appl water sttd summ, warning = FALSE}
summary(m_water_q_appl_sttd)

visreg(m_water_q_appl_sttd, xvar = "DailyAvgNetFlow", gg = TRUE) + 
  theme_bw() + 
  geom_point(size = 1)

visreg(m_water_q_appl_sttd, xvar = "TotalApplStd", gg = TRUE) + 
  theme_bw() + 
  geom_point(size = 1)
```
...
```{r lm flow appl prior water sttd summ, warning = FALSE}
summary(m_water_q_appl_pr_sttd)

visreg(m_water_q_appl_pr_sttd, xvar = "DailyAvgNetFlow", gg = TRUE) + 
  theme_bw() + 
  geom_point(size = 1)

visreg(m_water_q_appl_pr_sttd, xvar = "TotalApplStd_lag1", gg = TRUE) + 
  theme_bw() + 
  geom_point(size = 1)
```
