...
```{r gam flow2 backfit to observed values}
df_m_gam_flow2_lag2_fit <- df_chla_c2_lag %>% 
  drop_na(lag1, lag2) %>% 
...
```{r gam flow2 emmeans}
em_gam_sta_yr <- emmeans(m_gam_flow2_lag2, ~ StationCode | Year_fct)
...
em_gam_yr_sta <- emmeans(m_gam_flow2_lag2, ~ Year_fct | StationCode)
...
# Model 2: GAM Model with Flow and 2-way interactions for RD22
...
```{r gam flow2 no autocorr, warning = FALSE}
m_gam_flow2_RD22 <- gam(
  Chla_log ~ Year_fct*Flow + s(Week, bs = "cc", k = 5), 
  data = filter(df_chla_c2, StationCode=="RD22"),
  method = "REML", 
  knots = list(week = c(0, 52))
)
```
...
```{r gam flow2 no autocorr diag, warning = FALSE}
summary(m_gam_flow2_RD22)
appraise(m_gam_flow2_RD22)
shapiro.test(residuals(m_gam_flow2_RD22))
k.check(m_gam_flow2_RD22)
draw(m_gam_flow2_RD22, select = 1, residuals = TRUE, rug = FALSE)
plot(m_gam_flow2_RD22, pages = 1, all.terms = TRUE)
acf(residuals(m_gam_flow2_RD22))
Box.test(residuals(m_gam_flow2_RD22), lag = 20, type = 'Ljung-Box')
```

Shapiro-Wilk normality test shows that the residuals are normal, the diagnostic plots look really good. Box-LJung indicates no serial autocorrelation.

#backfit values to observed values and run emmeans
```{r gam flow2 backfit to observed values}
df_m_gam_flow2_RD22_fit <- df_chla_c2 %>% filter(StationCode=="RD22")%>%
  fitted_values(m_gam_flow2_RD22, data = .) %>% 
  mutate(fitted_bt = exp(.fitted) / 1000)

plt_m_gam_flow2_RD22_fit <- df_m_gam_flow2_RD22_fit %>% 
  ggplot(aes(x = fitted_bt, y = Chla)) +
  geom_point() +
  geom_abline(slope = 1, intercept = 0, color = "red") +
  theme_bw() +
...
plt_m_gam_flow2_RD22_fit


plt_m_gam_flow2_RD22_fit + facet_wrap(vars(Year_fct), scales = "free")
```

```{r gam flow2 emmeans}
em_gam_RD22_yr <- emmeans(m_gam_flow2_RD22, ~ Year_fct)

# Tukey post-hoc contrasts
pairs(em_gam_RD22_yr)

# Create table of contrasts and convert it to a tibble for plot
df_gam_RD22_yr <- em_gam_RD22_yr %>% 
  cld(sort = FALSE, Letters = letters) %>% 
  as_tibble() %>% 
  mutate(
    group = str_remove_all(.group, fixed(" ")),
...
# Create boxplot showing Tukey post-hoc results
plt_gam_RD22_yr <- df_gam_RD22_yr %>% 
  ggplot(
    aes(
      x = Year_fct,
      y = emmean,
...
# Model 2: GAM Model with Flow and 2-way interactions for STTD
...
```{r gam flow2 no autocorr, warning = FALSE}
m_gam_flow2_STTD <- gam(
  Chla_log ~ Year_fct*Flow + s(Week, bs = "cc", k = 5), 
  data = filter(df_chla_c2, StationCode=="STTD"),
  method = "REML", 
  knots = list(week = c(0, 52))
)
```
...
```{r gam flow2 no autocorr diag, warning = FALSE}
summary(m_gam_flow2_STTD)
appraise(m_gam_flow2_STTD)
shapiro.test(residuals(m_gam_flow2_STTD))
k.check(m_gam_flow2_STTD)
draw(m_gam_flow2_STTD, select = 1, residuals = TRUE, rug = FALSE)
plot(m_gam_flow2_STTD, pages = 1, all.terms = TRUE)
acf(residuals(m_gam_flow2_STTD))
Box.test(residuals(m_gam_flow2_STTD), lag = 20, type = 'Ljung-Box')
```
...
#backfit values to observed values and run emmeans
```{r gam flow2 backfit to observed values}
df_m_gam_flow2_STTD_fit <- df_chla_c2 %>% filter(StationCode=="STTD")%>%
  fitted_values(m_gam_flow2_STTD, data = .) %>% 
  mutate(fitted_bt = exp(.fitted) / 1000)

plt_m_gam_flow2_STTD_fit <- df_m_gam_flow2_STTD %>% 
  ggplot(aes(x = fitted_bt, y = Chla)) +
  geom_point() +
  geom_abline(slope = 1, intercept = 0, color = "red") +
  theme_bw() +
...
plt_m_gam_flow2_STTD_fit


plt_m_gam_flow2_STTD_fit + facet_wrap(vars(Year_fct), scales = "free")
```

```{r gam flow2 emmeans}
em_gam_STTD_yr <- emmeans(m_gam_flow2_STTD, ~ Year_fct)

# Tukey post-hoc contrasts
pairs(em_gam_STTD_yr)

# Create table of contrasts and convert it to a tibble for plot
df_gam_STTD_yr <- em_gam_RD22_yr %>% 
  cld(sort = FALSE, Letters = letters) %>% 
  as_tibble() %>% 
  mutate(
    group = str_remove_all(.group, fixed(" ")),
...
# Create boxplot showing Tukey post-hoc results
plt_gam_STTD_yr <- df_gam_STTD_yr %>% 
  ggplot(
    aes(
      x = Year_fct,
      y = emmean,
