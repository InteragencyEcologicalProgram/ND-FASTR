...
Formula: `r format(m_lm_cat3_wt_lag1$call) %>% str_c(collapse = "") %>% str_squish()` <br><br>
* Model 3 - `m_lm_cat3_lag1` - LM 3-way interactions without seasonal term <br>
Formula: `r format(m_lm_cat3_lag1$call) %>% str_c(collapse = "") %>% str_squish()` <br><br>
...
Formula: `r format(m_lm_cat2_wt_lag1$call) %>% str_c(collapse = "") %>% str_squish()` <br><br>
* Model 6 - `m_lm_cat2_lag1` - LM 2-way interactions without seasonal term <br>
Formula: `r format(m_lm_cat2_lag1$call) %>% str_c(collapse = "") %>% str_squish()` <br>
...
left_join(df_m_aic, df_m_bic, by = join_by(Model, df)) %>% arrange(BIC)
```

According to AIC, model 3 (LM 3-way interactions without seasonal term) was the best model, while BIC preferred model 6 (LM 2-way interactions without seasonal term). We'll look at both of these models more closely.

```{r export models and data}
# Export models and data used in the models
m_lm_cat2_lag1 %>% saveRDS(here("manuscript_synthesis/model/interim/chla_cat2_lm_model.rds"))
m_lm_cat3_lag1 %>% saveRDS(here("manuscript_synthesis/model/interim/chla_cat3_lm_model.rds"))
df_chla_wt_lag %>% saveRDS(here("manuscript_synthesis/model/interim/chla_model_data.rds"))
```
