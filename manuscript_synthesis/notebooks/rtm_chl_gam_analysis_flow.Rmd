...
m_chla_gam <- gam(
  Chla_log ~ (Year_fct + Flow + StationCode)^2 + s(DOY), 
  data = df_chla_c2,
  method = "REML"
)
```
...
f_chla_gam_k5 <- as.formula("Chla_log ~ (Year_fct + Flow + StationCode)^2 + s(DOY, k = 5)")
...
visreg(m_chla_gamm_k5_ar3_gam, xvar = "Flow", by = "StationCode", gg = TRUE) + 
  facet_wrap(vars(StationCode), scales = "free") +
  theme_bw() +
...
visreg(m_chla_gamm_k5_ar3_gam, xvar = "Flow", by = "StationCode", overlay = TRUE, gg = TRUE) +
  theme_bw() +
  ylab("log(Chlorophyll x 1000)") +
  xlab("Daily Average Flow (cfs)")
```
